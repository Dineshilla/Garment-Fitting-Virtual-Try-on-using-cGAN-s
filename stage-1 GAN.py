...
# %%
PATH = Path(r"E:\Semester-7\Mini Project\Review-2\Data_order")
...
def tf_load_image_train(image_file):
    ...
    full_image.set_shape([256, 192, 15])

    input_image = full_image[:, :, :12]  # First 12 channels
    ground_truth = full_image[:, :, 12:] # Last 3 channels
...
def generator_loss(disc_generated_output, gen_output, target):
  ...
  total_gen_loss = gan_loss + (LAMBDA * l1_loss) + (context_loss)
...
#     # Adjust input shape for 12-channel input
#     inp = tf.keras.layers.Input(shape=[256, 192, 12], name='input_image')
#     tar = tf.keras.layers.Input(shape=[256, 192, 3], name='target_image')

#     # Concatenation results in 15 channels
#     x = tf.keras.layers.concatenate([inp, tar])  # (batch_size, 256, 192, 15)

#     # Adjust the downsampling layers
#     down1 = downsample(64, 4, apply_batchnorm=False)(x)  # (batch_size, 128, 96, 64)
#     down2 = downsample(128, 4)(down1)  # (batch_size, 64, 48, 128)
#     down3 = downsample(256, 4)(down2)  # (batch_size, 32, 24, 256)

#     # Remaining layers are kept the same
#     zero_pad1 = tf.keras.layers.ZeroPadding2D()(down3)  # (batch_size, 34, 26, 256)
#     conv = tf.keras.layers.Conv2D(512, 4, strides=1, kernel_initializer=initializer, use_bias=False)(zero_pad1)  # (batch_size, 31, 23, 512)

#     batchnorm1 = tf.keras.layers.BatchNormalization()(conv)
#     leaky_relu = tf.keras.layers.LeakyReLU()(batchnorm1)

#     zero_pad2 = tf.keras.layers.ZeroPadding2D()(leaky_relu)  # (batch_size, 33, 25, 512)

#     last = tf.keras.layers.Conv2D(1, 4, strides=1, kernel_initializer=initializer)(zero_pad2)  # (batch_size, 30, 22, 1)
...
def Discriminator():
    ...
    # Adding more downsampling layers
    down1 = downsample(64, 4, apply_batchnorm=False)(x)    # (batch_size, 128, 96, 64)
    down2 = downsample(128, 4)(down1)                      # (batch_size, 64, 48, 128)
    down3 = downsample(256, 4)(down2)                      # (batch_size, 32, 24, 256)
    down4 = downsample(512, 4)(down3)                      # (batch_size, 16, 12, 512)
    down5 = downsample(512, 4)(down4)                      # (batch_size, 8, 6, 512)

    # Corresponding upsampling layers
    up1 = upsample(512, 4)(down5)                          # (batch_size, 16, 12, 512)
    up2 = upsample(256, 4)(up1)                            # (batch_size, 32, 24, 256)

    # Concatenation for skip connections (if required)
    concat1 = tf.keras.layers.concatenate([up2, down3])    # (batch_size, 32, 24, 512)

    zero_pad1 = tf.keras.layers.ZeroPadding2D()(concat1)   # (batch_size, 34, 26, 512)
    conv = tf.keras.layers.Conv2D(512, 4, strides=1, kernel_initializer=initializer, use_bias=False)(zero_pad1)  # (batch_size, 31, 23, 512)
...
# %%
checkpoint_dir = './training_checkpoints_order'
...
# def generate_images(model, test_input, tar):
#   prediction = model(test_input, training=True)
#   plt.figure(figsize=(20, 10))  # Adjust the figure size to accommodate the extra plot

#   # Add the new plot for input[:,:,3:]
#   display_list = [test_input[0][:,:,:3], test_input[0][:,:,3:], tar[0], prediction[0]]
#   title = ['Input Image RGB', 'Input Image Alpha Channel', 'Ground Truth', 'Predicted Image']
